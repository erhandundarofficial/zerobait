...
async function virusTotalLookup(u: string): Promise<any> {
  ...
  try {
    ...
    return { error: e?.message || 'virustotal_failed' }
  }
}
  ...
  try {
    ...
    if (cached) {
      ...
      if (ageMs >= 0 && ageMs < ttlMs) {
        ...
        return { ...(data as AnalysisResult), from_cache: true }
      }
    }
  } catch {}
